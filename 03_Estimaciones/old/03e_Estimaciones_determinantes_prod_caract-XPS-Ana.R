...
# Ultima fecha de modificacion: 24 feb, 2022
...
fun_reg <- function(data, reg_def, tag, sample = NA){
  ...
  coefs <- c(
    ...
    "Crédito - al menos 1 crédito de fuente formal" = 'cred1', 
...
table(data_prod$c11, data_prod$c4)
table(data_prod$c5)
table(data_prod$ong, data_prod$agrario)

informal = ifelse((almacen == 1 | prestamista == 1), 1, 0)
cred_form = ifelse((informal == 0) & (banco == 1 | otros == 1 | coo == 1 | ong == 1 | gob == 1 | int == 1))


# Participacion por tamano de upa sobre el total de la muestra
table(data_prod$c11, data_prod$tipo_upa)
428812/nrow(data_prod)
146098/nrow(data_prod)
62275/nrow(data_prod)
17169/nrow(data_prod)

asis <- data_prod %>% dplyr::filter(a1 == 1)
nrow(asis)/nrow(data_prod)
table(asis$tipo_upa)
161702/nrow(asis)
161702/nrow(data_prod %>% dplyr::filter(tipo_upa == "Microfundio"))

cred <- data_prod %>% dplyr::filter(c11 == 1)
table(cred$tipo_upa)
425756/nrow(data_prod %>% dplyr::filter(tipo_upa == "Microfundio"))
26323/nrow(data_prod %>% dplyr::filter(tipo_upa == "Pequeña"))
9887/nrow(data_prod %>% dplyr::filter(tipo_upa == "Mediana"))
...
# Arroz
arroz <- fun_reg(data = data_prod, reg_def = "Productividad agregada", tag = 'Maíz (blanco o amarillo)')

# Maiz
maiz <- fun_reg(data = data_prod, reg_def = "Productividad agregada", tag = 'Arroz')
